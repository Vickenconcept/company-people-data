<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <button
            ...
            class="px-6 py-2.5 rounded-2xl bg-gradient-to-r from-[#8f47f2] to-[#5d30dc] text-white font-semibold hover:opacity-95 transition-all shadow-sm shadow-violet-300/60 cursor-pointer no-underline"
        >
            Create Template
        </button>
    </div>
    ...
    @if($templates->count() > 0)
        <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            @foreach($templates as $template)
                <div class="group rounded-3xl border border-violet-100/80 bg-white shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
                    <div class="p-6">
                            ...
                            <div class="flex-1">
                                <div class="flex items-center gap-2 mb-1">
                                    ...
                                    @if($template->is_default)
                                        <span class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold bg-gradient-to-r from-[#8f47f2] to-[#5d30dc] text-white rounded-full shadow-sm">
                    ...
                    <div class="flex gap-2 p-4 bg-slate-50/90 border-t border-violet-100/70">
                        <button 
                            wire:click="openModal({{ $template->id }})"
                            class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 text-sm font-semibold bg-violet-700 text-white rounded-2xl hover:bg-violet-800 transition-colors cursor-pointer"
                        >
                        ...
                        <button 
                            ...
                            class="inline-flex items-center justify-center px-3 py-2 text-sm font-semibold bg-red-500 text-white rounded-2xl hover:bg-red-600 transition-colors cursor-pointer"
                        >
    ...
    @else
        <div class="py-16 text-center rounded-3xl border border-violet-100/80 bg-white shadow-sm">
            <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-violet-100 mb-4">
                <svg class="w-8 h-8 text-violet-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
            </div>
            <p class="text-slate-600 mb-4 font-medium">No email templates yet.</p>
            <button
                wire:click="openModal"
                class="px-6 py-2.5 rounded-2xl bg-gradient-to-r from-[#8f47f2] to-[#5d30dc] text-white font-semibold hover:opacity-95 transition-all shadow-sm shadow-violet-300/60 cursor-pointer"
            >
    ...
    @if($showModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" wire:click="closeModal">
            <div class="bg-white rounded-3xl max-w-2xl w-full max-h-[88vh] flex flex-col shadow-2xl" wire:click.stop>
                <!-- Modal Header - Fixed -->
                    ...
                    <div class="space-y-5">
                        <div>
                            ...
                            <input 
                                type="text"
                                wire:model="name"
                                class="w-full border border-violet-100 bg-violet-50/30 rounded-2xl px-3 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:border-violet-500 focus:bg-white focus:ring-2 focus:ring-violet-500/20 focus:outline-none transition-colors"
                        ...
                        <div>
                            ...
                            <textarea 
                                wire:model="body"
                                rows="6"
                                class="w-full border border-violet-100 bg-violet-50/30 rounded-2xl px-3 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:border-violet-500 focus:bg-white focus:ring-2 focus:ring-violet-500/20 focus:outline-none transition-colors resize-none"
                        ...
                        <div class="rounded-2xl border border-violet-100 bg-violet-50/30 px-3 py-2.5">
                            <label class="flex items-center gap-2.5 cursor-pointer group">
                                <input 
                                    type="checkbox"
                                    wire:model="is_default"
                                    class="w-4 h-4 rounded border-slate-300 text-violet-600 focus:ring-2 focus:ring-violet-500/20 cursor-pointer"
                                >
                ...
                <!-- Modal Footer - Fixed -->
                <div class="flex justify-end gap-3 px-6 py-4 border-t border-violet-100/70 bg-slate-50 flex-shrink-0 rounded-b-3xl">
                    <button 
                        wire:click="closeModal"
                        class="px-4 py-2 text-sm font-semibold bg-white border border-violet-100 text-slate-700 rounded-2xl hover:bg-violet-50/40 hover:border-violet-200 transition-colors cursor-pointer"
                    >
                        Cancel
                    </button>
                    <button 
                        wire:click="save"
                        class="px-4 py-2 text-sm font-semibold bg-gradient-to-r from-[#8f47f2] to-[#5d30dc] text-white rounded-2xl hover:opacity-95 transition-all shadow-sm shadow-violet-300/60 cursor-pointer"
                    >
